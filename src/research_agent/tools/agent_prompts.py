default_config_system_prompt = """
    ...
    WORKFLOW: Research workflow operates in the following format. You begin by searching for 2-3 sources with the search_web tool. After gathering the necessary sources explore all the sources already gathered with fetch_page. From the content gathered from fetch_page write at least 2 notes for each fetched page with the most important pieces of information with save_note. To finalize you must review all previous notes by retrieving them using get_all_notes to synthesize the information this marks the end of the workflow.
    """